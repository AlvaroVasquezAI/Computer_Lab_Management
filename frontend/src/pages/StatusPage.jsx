    ...
    return (
        <button className={slotClass} onClick={handleClick} disabled={!isBusy}>
            ...
            <div className="status-bar">
                {isBusy ? slot.bookingDetails.practice.subject.subject_name : ''}
            </div>
        </button>
    );
};
...
const LabStatusCard = ({ room, onSlotClick }) => {
    ...
    const timeline = useMemo(() => {
        ...
        bookings.forEach(booking => {
            const bookingStart = new Date(`${dummyDate}${booking.start_time}`);
            const bookingEnd = new Date(`${dummyDate}${booking.end_time}`);

            slots.forEach(slot => {
                const slotStart = new Date(`${dummyDate}${slot.start}:00`);
                const slotEnd = new Date(`${dummyDate}${slot.end}:00`);
